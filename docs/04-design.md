...
| テスト | 手法 |
|--------|------|
| サンプル収集 | セッションID/CSRF/リセットトークンを数千〜数万件収集 |
| NIST SP 800-22テスト | 周波数テスト、ランテスト、エントロピー推定 |
| LCG検出 | 線形合同生成器の下位ビット周期性パターン |
| 時刻ベースシード | トークン値と生成時刻の相関分析 |
...
| InsecureRandom | CH6 | NIST統計テストで検出 |
...
| WAF/レートリミット | スローペースモード、正規リクエストに見せかけるプローブ設計 |
